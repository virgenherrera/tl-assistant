...
import type { DogmaBrief, DogmaRefreshResult, UniversalAgentContext } from '#domain/dogma';
import type { DogmaRepositoryPort } from '#ports';

@Injectable()
export class LocalDogmaRepositoryService implements DogmaRepositoryPort {

  async saveRefreshResult(result: DogmaRefreshResult, outputDir = '.tl-assistant/dogma'): Promise<readonly string[]> {
    ...
    const briefJsonPath = path.join(outputDir, 'dogma-brief.json');
    const briefMarkdownPath = path.join(outputDir, 'dogma-brief.md');
    const agentContextJsonPath = path.join(outputDir, 'agent-context.json');
    const agentContextMarkdownPath = path.join(outputDir, 'AGENT_CONTEXT.md');
    ...
    await fs.writeFile(briefMarkdownPath, renderDogmaBriefMarkdown(persistedResult.brief), 'utf8');
...
async function removeLegacyArtifacts(outputDir: string): Promise<void> {
  await Promise.all([
    fs.rm(path.join(outputDir, 'dogma.json'), { force: true }),
    fs.rm(path.join(outputDir, 'dogma.md'), { force: true }),
  ]);
}

function renderDogmaBriefMarkdown(brief: DogmaBrief): string {
...
function renderUniversalAgentContextMarkdown(context: UniversalAgentContext): string {
  ...
  return [
    ...
    `- Dogma brief: \`${context.dogmaBriefRef}\``,
    ...
    '',
    '## Reglas de fuentes',
    '- Citar fuentes como `foundation://ruta/relativa.ext`.',
