...
import type { DogmaBriefItem, DogmaBriefItemKind } from '#domain/dogma';
import { masterPersonaDirectives } from '#dogma-context/persona';

@Injectable()
export class LocalDogmaAgentService implements AgentRuntimePort {
  readonly name = masterPersonaDirectives.name;

  async readForDogma(request: AgentReadRequest): Promise<AgentReadResult> {
...
function inferBriefKind(text: string): DogmaBriefItemKind {
