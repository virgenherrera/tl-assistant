...
export class UnsupportedDocumentExtractor implements DocumentExtractorPort {
  ...
  extract(source: FoundationSource): Observable<DocumentExtractionEvent> {
    ...
    if (imageExtensions.has(source.extension)) {
      return new Observable<DocumentExtractionEvent>((subscriber) => {
        subscriber.error(new Error(`Imagen requiere extractor OCR/visión antes de tratarse como foundation: ${source.extension}`));
      });
    }
...
export class LocalDocumentExtractorRegistry {
  ...
  extractorFor(source: FoundationSource): DocumentExtractorPort {
    return [
